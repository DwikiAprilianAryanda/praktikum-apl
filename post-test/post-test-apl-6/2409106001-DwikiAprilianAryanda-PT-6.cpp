...
// Fungsi Bubble Sort untuk nama secara ascending
void bubbleSortNama(User* user) {
    for (int i = 0; i < user->jumlahProduk - 1; i++) {
        for (int j = 0; j < user->jumlahProduk - i - 1; j++) {
            if (user->daftarProduk[j].nama > user->daftarProduk[j + 1].nama) {
                Produk temp = user->daftarProduk[j];
                user->daftarProduk[j] = user->daftarProduk[j + 1];
                user->daftarProduk[j + 1] = temp;
            }
        }
    }
}

// Fungsi Selection Sort untuk harga secara descending
void selectionSortHarga(User* user) {
    for (int i = 0; i < user->jumlahProduk - 1; i++) {
        int maxIdx = i;
        for (int j = i + 1; j < user->jumlahProduk; j++) {
            if (user->daftarProduk[j].harga > user->daftarProduk[maxIdx].harga) {
                maxIdx = j;
            }
        }
        if (maxIdx != i) {
            Produk temp = user->daftarProduk[i];
            user->daftarProduk[i] = user->daftarProduk[maxIdx];
            user->daftarProduk[maxIdx] = temp;
        }
    }
}

// Fungsi Insertion Sort untuk kode secara ascending
void insertionSortKode(User* user) {
    for (int i = 1; i < user->jumlahProduk; i++) {
        Produk key = user->daftarProduk[i];
        int j = i - 1;
        while (j >= 0 && user->daftarProduk[j].kode > key.kode) {
            user->daftarProduk[j + 1] = user->daftarProduk[j];
            j--;
        }
        user->daftarProduk[j + 1] = key;
    }
}

// Prosedur untuk menampilkan data produk dengan pointer dan opsi sorting
void tampilDataProduk(User* user) {
    ...
    cout << "1. Nama (Ascending, Bubble Sort)" << endl;
    cout << "2. Harga (Descending, Selection Sort)" << endl;
    cout << "3. Kode (Ascending, Insertion Sort)" << endl;
    cout << "Pilih opsi (1-3): ";
    cin >> sortChoice;
    ...
    if (sortChoice == 1) {
        ...
        selectionSortHarga(user);
    } else if (sortChoice == 3) {
        insertionSortKode(user);
    } else {
